          ...
          <div className="relative" ref={menuRef}>
            ...
            {showGameMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-[#0d1117] border border-white/5 rounded-2xl shadow-2xl z-50 py-2 animate-in fade-in zoom-in-95 duration-100">
                <button
                  onClick={() => { setSelectedGame(''); setMatches([]); setCurrentIndex(0); fetchMatches(''); setShowGameMenu(false); }}
                  className={cn(
                ...
                {user?.library?.map((game) => (
                  <button
                    key={game.gameName}
                    onClick={() => { setSelectedGame(game.gameName); setMatches([]); setCurrentIndex(0); fetchMatches(game.gameName); setShowGameMenu(false); }}
                    className={cn(
          ...
          {!isSearching && matches.length === 0 ? (
             <Card className="h-full w-full border-2 border-dashed border-white/5 bg-secondary/5 rounded-[3rem] flex flex-col items-center justify-center text-center p-12 space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter">ARAMAYA BAŞLA</h3>
                  <p className="text-sm text-muted-foreground font-medium max-w-xs mx-auto">
                    Yeni oyun arkadaşları bulmak için yukarıdaki "BUL" butonuna bas!
                  </p>
                </div>
             </Card>
